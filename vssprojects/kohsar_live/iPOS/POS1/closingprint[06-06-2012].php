...
<table width="300" style="font-size:9px;font-family:Lucida Sans Unicode, Lucida Grande, sans-serif; border-collapse:collapse; border-color:#FFF;margin-top:10px;" class="simple">
  ...
  // displaying credit bills //changed $dbname_main to $dbname_detail on line 344, 346, 348 by ahsan 22/02/2012
	...
	$query	=	"SELECT
			  ...
			  (SELECT sum(saleprice * quantity)-(SELECT SUM(globaldiscount) FROM $dbname_detail.sale sg WHERE s1.fkaccountid=id AND sg.fkclosingid='$closingsession' AND s.pksaleid = sg.pksaleid) as subtotal FROM $dbname_detail.saledetail,$dbname_detail.sale s1 WHERE s1.fkaccountid =  id AND pksaleid = fksaleid AND s1.fkclosingid='$closingsession' AND s.pksaleid = s1.pksaleid)

			  -

			  (SELECT (IF(sum(amount)IS NULL,0,sum(amount))) as am FROM $dbname_detail.payments ,$dbname_detail.sale s1 WHERE s1.fkaccountid =  id AND pksaleid = fksaleid AND s1.fkclosingid='$closingsession' AND s.pksaleid = s1.pksaleid AND paymenttype<>'c'),2) as totalcredit

		FROM 

				$dbname_detail.sale s,$dbname_detail.account LEFT JOIN $dbname_detail.addressbook ON fkaddressbookid=pkaddressbookid
...
<table width="300" style="font-size:9px;font-family:Lucida Sans Unicode, Lucida Grande, sans-serif; border-collapse:collapse; border-color:#FFF;" class="simple">
  ...
  <tr>
    ...
    <th colspan="4">Canceled Sales</th>
  ...
  <tr>
    ...
    <td width="75"><?php echo $cancelsalesarray[$i][pksaleid];?></td>

    <td width="75"><?php echo $cancelsalesarray[$i][dtime];?></td>

    <td width="75" align="right"><?php echo numbers($cancelsalesarray[$i][totalamount]);?></td>    

    <td width="75"><?php echo $cancelsalesarray[$i][itemnum];?></td>
...
<?php
	...
	$cashcollectquery	=	"SELECT 
							...
							c.pkpaymentid,from_unixtime(paytime,'%H:%i:%s') as dtime,SUM(c.amount) as amount,CONCAT(a.firstname,' ',a.lastname) as name,t.title cname

						FROM 

							$dbname_detail.payments c, $dbname_detail.sale s, $dbname_detail.account t, $dbname_detail.addressbook a
...
<table width="100%" style="font-size:9px;font-family:Lucida Sans Unicode, Lucida Grande, sans-serif; border-collapse:collapse; border-color:#FFF;" class="simple">
  ...
  <tr>
    ...
    <td><?php echo $cancelsalesarray[$i][pksaleid];?></td>

    <td><?php echo $cancelsalesarray[$i][dtime];?></td>

    <td align="right"><?php echo numbers($cancelsalesarray[$i][totalamount]);?></td>

    <td><?php echo $cancelsalesarray[$i][itemnum];?></td>
