...
{//changed $dbname_main to $dbname_detail on line 190 by ahsan 22/02/2012
  $salerows	=	$AdminDAO->getrows("$dbname_detail.saledetail",'pksaledetailid,fkdiscountid,fkstockid,saleprice, sum( quantity ) AS quantity, sum( saleprice * quantity ) AS subtotal, boxsize'," fksaleid='$tempsaleid' group by fkstockid,saleprice,fkdiscountid ORDER BY pksaledetailid DESC");
}
  $items=0;
