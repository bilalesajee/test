...
where s.fkclosingid in ({$Closingidz}) and s.status = 1 and s.fkclosingid=cf.pkclosingid and s.datasent=0 group by case s.fkaccountid when '0' then 0 else 1 end
having sale > 0 ";
...
where ap.fkclosingid in ({$Closingidz})
