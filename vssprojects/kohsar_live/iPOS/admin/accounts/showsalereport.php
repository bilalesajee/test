...
FROM $dbname_detail.sale s left join $dbname_detail.saledetail sd on (s.pksaleid=sd.fksaleid) WHERE month(FROM_UNIXTIME( s.updatetime, '%Y-%m-%d' ) )='$month' and year( FROM_UNIXTIME(s.updatetime, '%Y-%m-%d' ))='$year'  and s.status = '1'  group by  FROM_UNIXTIME( s.updatetime, '%m-%Y' )   ";

$query10 = $AdminDAO->queryresult($q0);
$q="select SUM(p.amount) as cash      
FROM $dbname_detail.sale s left join $dbname_detail.payments p on (s.pksaleid=p.fksaleid)  WHERE month(FROM_UNIXTIME( s.updatetime, '%Y-%m-%d' ) )='$month' and year( FROM_UNIXTIME(s.updatetime, '%Y-%m-%d' ))='$year'  and s.status = '1' and p.paymentmethod='c' group by  FROM_UNIXTIME( s.updatetime, '%m-%Y' )   ";
...
FROM $dbname_detail.sale s left join $dbname_detail.payments p on (pksaleid=fksaleid) WHERE month(FROM_UNIXTIME( s.updatetime, '%Y-%m-%d' ) )='$month' and year( FROM_UNIXTIME(s.updatetime, '%Y-%m-%d' ))='$year'  and s.status = '1' and p.paymentmethod='fc' group by  FROM_UNIXTIME( s.updatetime, '%m-%Y' ) ";
