...
$discounts	=	$AdminDAO->getrows("$dbname_detail.sale","pksaleid,round(globaldiscount,2) as discount,round(totalamount,2) totalamount,from_unixtime(updatetime,'%H:%i:%s') as dtime","fkclosingid='$closingsession' AND status=1 AND globaldiscount>0 and fkcouponid=0");
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
 $query = "SELECT 	c.pkcouponid  as pkcouponid,c.amount,case c.paymentmethod when 'c' then 'Cash' when 'cc' then 'Credit Card' when 'fc' then 'Foreign Currency' else 'Cheque' end paymentmethod0,c.reason FROM
	$dbname_detail.coupon_management c 
	where  c.fkclosingid = '$closingsession' order by pkcouponid desc ";
   $result_query_adv		=	$AdminDAO->queryresult($query);
   $query = "SELECT 	sum(c.amount) amt FROM $dbname_detail.coupon_management c  	where  c.fkclosingid = '$closingsession'";
   $result_query_adv_sum		=	$AdminDAO->queryresult($query);
    $advbk=numbers($result_query_adv_sum[0]['amt']);
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
?>
...
<table width="300" style="font-size:9px;font-family:Lucida Sans Unicode, Lucida Grande, sans-serif; border-collapse:collapse; border-color:#FFF;margin-top:10px;" class="simple">
  ...
  <tr>
    ...
    <th align="right"><?php $netsales	=	($closingarray[0]['totalsale']+$advbk)-$tdisk; echo numbers($netsales);?></th>
  ...
  <tr>
    ...
    <th align="right"><?php /* // commented by Yasir 23-06-11 echo numbers($closingarray[0]['cashdiffirence']-$totalcashcollection);*/ echo numbers($closingarray[0]['cashdiffirence']+$cashdiff-$advbk); // +$cashdiff added by Yasir 26-07-11 
...
<tr>
  ...
  <tr>
    ...
    <td align="right"><?php echo $advbk;?></td>
  ...
  <tr>
    ...
    <td align="right"><?php echo numbers($closingarray[0]['cashsale']+$advbk-$cashdiff+$closingarray[0]['cashcollect']); // -$cashdiff added by Yasir 26-07-11?></td>
...
//$closingsession
if(sizeof($result_query_adv)>0)

{
 
?>
<table width="100%" class="simple" style="font-size:9px;font-family:Arial, Helvetica, sans-serif; border-collapse:collapse; border-color:#FFF;">
  <tr> <th colspan="4">Advance Booking</th> </tr>
  <tr>  <td width="70">CouponID</td>
...
for($i=0;$i<sizeof($result_query_adv);$i++)

{

	  $total_amount+=	$result_query_adv[$i]['amount'];
	

?>
  <tr>
    <td><?php echo $result_query_adv[$i]['pkcouponid'];?></td>
    <td><?php echo $result_query_adv[$i]['paymentmethod0'];?></td>
    <td><?php echo $result_query_adv[$i]['reason'];?></td>
 
    <td align="right"><?php echo $result_query_adv[$i]['amount'];?></td>
  </tr>
  <?php
...
<?php
	  ...
	  $query = "SELECT 	pksaleid,c.pkcouponid as coupon_id,c.amount   FROM 	$dbname_detail.sale s   left join $dbname_detail.coupon_management c on c.pkcouponid = s.fkcouponid
	  where  c.status='2' and c.fkclosingid = '$closingsession' group by s.fkcouponid"; 
...
<table width="100%" class="simple" style="font-size:9px;font-family:Arial, Helvetica, sans-serif; border-collapse:collapse; border-color:#FFF;">
  <tr>
    <th colspan="3">Coupon Used </th>
  </tr>
  <tr>
    <td width="176">CouponID</td>
    <td width="176">SaleID</td>
...
?>
  <tr>
    <td><?php echo $result_query[$i]['coupon_id'];?></td>
     <td><?php echo $result_query[$i]['pksaleid'];?></td>
