...
if($customerid !="-1")
{
	if($_SESSION['siteconfig']!=1){//edit by ahsan 17/02/2012, added if condition
		...
		$sql="SELECT 
				...
				FROM
					customer c LEFT JOIN $dbname_detail.addressbook  ON (c.fkaddressbookid = pkaddressbookid) 
				WHERE
...
<?php 		if($_SESSION['siteconfig']!=1){//edit by ahsan 17/02/2012, added if condition?>
    <tr id="adjustmentrow" style="display:none">
	 ...
	 <tr id="writeoff" style="display:none" >
	 ...
	 <tr id="favouredby" style="display:none" >
		<td>Favoured By</td>				
		<td valign="top">
