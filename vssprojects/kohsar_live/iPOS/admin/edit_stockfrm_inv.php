...
<legend>Edit Invoice Stock</legend>

<div  style="float:right;">

<span class="buttons">

<?php if($invc_status==0 || $gid==6){?>
...
<?php if($_SESSION['addressbookid']== '1888')
	{
		?>
<button type="button" class="positive" onclick="openWin()">
...
<?php if($invc_status==0  || $gid==6){?>
  <td  style="width:105px;"> <button type="button" class="datepicker_control" onclick="del_stock(<?php echo $result2[$i]['pkstockid'];?>);">Delete </button></td>
<?php }?>
</tr>
...

{
	...
	options	=	{	
					...
					url : 'insert_invoice_stock_new_inv.php?id='+id,
...
function del_stock(pkpurchasereturndetailid)
...
if (result==true) {
	...
	var data = 'pkstockid='+pkpurchasereturndetailid;

	$.ajax({

  type: "POST",

  url: 'delete_inv_stk.php',
  ...
  success: function(response) {  
	...
	jQuery('#invoiceform').load('edit_stockfrm_inv.php?id='+'<?php echo $id; ?>&param=undefined');
...
function openWin()
{
myWindow=window.open('history_print.php?screen=stock&ids=<?php echo $id?>',"myWin","menubar,scrollbars,left=30px,top=40px,height=800px,width=1024px");
