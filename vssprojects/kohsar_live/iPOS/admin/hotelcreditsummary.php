...
///////////////////////////////////////////////////////////////////////////////////
	...
	$customers		=	$AdminDAO->getrows("customer","ctype","pkcustomerid='$cid'");
...
<?php
	...
	$customersql="SELECT 
						...
						companyname as title,taxnumber
	...
	$todate				=	@mktime(23,59,59,$tomon,$today,$toyr);
		 ...
		 $query		=	"SELECT paymentmethod as type,pksaleid,serialno, SUM(amount)  amount,FROM_UNIXTIME(paytime,'%d-%m-%y') trdatetime FROM $dbname_detail.sale,$dbname_detail.payments where fkaccountid='$cid' and fksaleid=pksaleid and paymenttype='c' and paytime < '$fromdate' group by fkaccountid
...
";
		...
		// $AdminDAO->dq=1;
	$balance_total		=	$AdminDAO->getrows("$dbname_detail.sale s,$dbname_detail.saledetail sd,customer c","sum((sd.saleprice*sd.quantity)+sd.taxamount * case when sd.taxable = 1 then 0 else 1 end ) as taxamount","s.fkaccountid=pkcustomerid AND s.status=1 AND  pkcustomerid='$cid' AND sd.fksaleid=pksaleid AND s.datetime < '$fromdate'");
$op			=	$balance_total[0]['taxamount'];
$grandtotal = $op;
//$AdminDAO->dq=0;
		//exit;
...
//code added by jafer 14-12-11
		...
		$customerinfo		=	$AdminDAO->getrows("$dbname_detail.sale s,$dbname_detail.saledetail sd,customer c","s.pksaleid,s.serialno,from_unixtime(s.datetime,'%d-%m-%y') as trdatetime,MONTH(from_unixtime(s.datetime,'%d-%m-%y')) AS m,sum((sd.saleprice*sd.quantity)+sd.taxamount * case when sd.taxable = 1 then 0 else 1 end ) as taxamount","s.fkaccountid=pkcustomerid AND s.status=1 AND  pkcustomerid='$cid' AND sd.fksaleid=pksaleid AND s.datetime BETWEEN '$fromdate' AND '$todate' GROUP BY pksaleid");
...
<table class="repstyle" style="width:8in;border-collapse:collapse;" cellpadding="0" cellspacing="0">
  ...
  <tr>
    ...
    <td style="border-left:1px solid #fff;border-right:1px solid #fff;border-bottom:1px solid #FFF;"><b><span style="text-transform:uppercase"><?php echo $companyname;?></span></b></td>
    <td align="right" style="border-left:1px solid #fff;border-right:1px solid #fff;border-bottom:1px solid #FFF;"><span style="text-transform:uppercase" > Opening balance <?php echo number_format($op,2);?></span></td>
    <td style="border-left:1px solid #fff;border-right:1px solid #fff;border-bottom:1px solid #FFF;">&nbsp;</td>
...
// echo $date	=	date('m-Y', $sdate);
...

		{
...

			{
				...
				$pksaleid			=	$customerinfo[$k]['serialno'];
  ...
  <?php
				...
				if($customerinfo[$k]['m']!=$customerinfo[$k+1]['m'] )

				{
					...
					?>
                    	  ...
                    	  <tr>
                            ...
                            <td colspan="3" align="right"><h3>Subtotal for <?php  echo date("F", mktime(0, 0, 0,$customerinfo[$k]['m'], 10)); //echo $currentmonth;?>: <?php echo number_format($subtotal,2);?></h3></td>
...
<?php if($customerinfo[$k+1]['m'] !=''){?>
                          ...
                          <tr>
                            ...
                            <td colspan="3"><h3><?php    echo date("F", mktime(0, 0, 0, $customerinfo[$k+1]['m'], 10)).' '.$currentyear;//echo $currentmonth."    ".$currentyear;?></h3></td>
...
<?php if($customerinfo[$k+1]['m'] !=''){?>
            ...
            <td colspan="3" align="right"><h3>Subtotal for <?php  echo $customerinfo[$k]['m'];?>: <?php echo number_format($subtotal,2);?></h3></td>
