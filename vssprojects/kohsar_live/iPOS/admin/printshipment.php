...
$query		=	"SELECT 
					...
					shiplistdetails.received,
					shiplistdetails.damaged,
					deadline,
					code3,
					statusname,
				...
				FROM 
					shiplist LEFT JOIN currency ON (fkcurrencyid=pkcurrencyid) LEFT JOIN addressbook ON (shiplist.fkaddressbookid=pkaddressbookid) LEFT JOIN store ON (fkstoreid=pkstoreid) LEFT JOIN countries ON (shiplist.fkcountryid=pkcountryid),statuses,shiplistdetails WHERE fkstatusid=pkstatusid AND pkshiplistid=shiplistdetails.fkshiplistid";
if($ids)
{
	$query		.=	" AND fkshipmentid IN ($ids) $status $sort";
...
<b>Shipment Details</b><br /><?php echo date("d:m:Y h:i:s a");?>
</div>
<div id="currencybox">
<?php echo $currencies;?>&nbsp;&nbsp;&nbsp;&nbsp;
...
<table width="900" align="left" style=" margin-left:0px; margin-right:auto;font-size:10px;font-family:Arial, Helvetica, sans-serif;">
  ...
  <tr bgcolor="<?php echo $color; ?>">
    ...
    <td><?php echo html_entity_decode($shipdata[$i]['itemdescription']);?></td>
    ...
    <td><?php echo $shipdata[$i]['received'];?></td>
   <td><?php echo $shipdata[$i]['damaged'];?></td>
  ...
  <?php
	...
	//echo " $priceinrs > and the totalprice is $totalprice<br>";
	if($currencyid)
	{
...
<script language="javascript">
	...
	{
		window.location	=	'printshipment.php?currency='+cid+'&<?php echo $qstring;?>';
	}
	function sendmail()
	{
