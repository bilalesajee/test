...
$date = $option;*/
 ...
 if($sdate != '' && $edate!='')
  {
   $cond = "sad.addtime  between '$sdate' and '$edate'"; 
  }
...
$query = "SELECT CONCAT(ad.firstname,' ',ad.lastname) AS fullname,b.itemdescription,b.barcode,quantity,orgquantity,if(type=0,'Addition','Subtraction') type from $dbname_detail.stock_adjustment_detail   left join $dbname_main.barcode b on (fkbarcodeid=pkbarcodeid) left join $dbname_detail.stock_adjustment sad on (pkstockadjustmentid=fkstockadjustmentid)  left join $dbname_main.addressbook ad on (pkaddressbookid=sad.add_by) where  $cond ";
...
<?php for($i=0;$i<count($reportresult);$i++){?>
  <tr>
    <td><?php echo $i+1;?>&nbsp;</td>
    ...
    <td><?php echo $reportresult[$i]['fullname'];?>&nbsp;</td>
  </tr>
  <?php 
