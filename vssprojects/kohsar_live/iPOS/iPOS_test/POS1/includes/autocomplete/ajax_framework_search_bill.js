...
/* -------------------------- */
var autodivid='';
function suggestnow_(e,acdivid,v1)
{
	...
	{
		//document.getElementById('results_').style.display='block';
		
		autosuggest_(v1);
	}
	autodivid=acdivid;
}
function autosuggest_(v) 
{
	
	//alert('hello');
	var q='';
	if( document.getElementById('productnameit'))
	{
		q = document.getElementById('productnameit').value;
	}	
...
//	alert(q+' is the q'+v);
	...
	{
		nocache = Math.random();
		http.open('get', 'includes/autocomplete/search_bills.php?q='+q+'&nocache = '+nocache);
		http.onreadystatechange = autosuggestReply_;
		http.send(null);
	}
	else
	{
		//alert('i am empty');
		if(document.getElementById('results_'))
		{
			var sdiv = document.getElementById('results_');
			sdiv.style.display="none";
		}
	}
}
function autosuggestReply_() 
{
	if(http.readyState == 4)
	{
		var response = http.responseText;
		e = document.getElementById('results_');
		if(response!="")
		{
...
{ 
	...
	{ 
		...
		document.serachitemfrm.bc.value	=	barcode;		
...
function move_up(to) 
{
	document.getElementById('productnameit').scrollTop = to;
}
function getlitext_(current,barcode)
{
	var litext	=	jQuery.trim(document.getElementById(current).textContent);
	//var val	=	document.getElementById(current).value;	
	document.getElementById('productnameit').value=litext;
