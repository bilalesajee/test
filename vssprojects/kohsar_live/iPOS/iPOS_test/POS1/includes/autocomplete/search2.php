<?php @session_start();
   if($_SESSION['countername']=='')
   {
   		$_SESSION['countername']=8;
   }
	// ---------------------------------------------------------------- // 
	...
	?>
		<li id="<?php echo $i;?>" value="<?php echo $barcodeidlist;?>">
              <a href="javascript:void(0)" onclick="getlitext('<?php echo $getRecord[$d]['itemdescription'];?>','<?php echo $barcodeidlist;?>','<?php echo $lid;?>')"   id="<?php echo $i;?>_l" class="<?php echo $barcodeidlist;?>"><?php echo ' <font color=blue> '.$getRecord[$d]['bc'].'  </font> <span id=itemname_'.$i.' class="'.$getRecord[$d]['itemdescription'].'">'.$getRecord[$d]['itemdescription'].'</span>';?></a>
        </li>
	<?php 
		$i++;
	} 
