...
class Bill
{
	...
	function getsaledetails($saleid)
	{
		...
		$saledetails		=	$this->billDAO->getrows("$dbname_detail.saledetail",'pksaledetailid,fkstockid,saleprice, sum( quantity ) AS quantity, sum( saleprice * quantity ) AS subtotal,boxsize,fkdiscountid,fkbarcodeid '," fksaleid='$saleid' AND quantity>0 GROUP BY `fkstockid`,`saleprice`,`fkdiscountid`");
		return $saledetails;
	}
	function getsaledetails2($saleid)
	{
		global $dbname_detail;
		//$saledetails		=	$this->billDAO->getrows("saledetail","*","fksaleid = '$saleid'","timestamp","DESC");
		$saledetails		=	$this->billDAO->getrows("$dbname_detail.saledetail",'pksaledetailid,fkstockid,saleprice, sum(quantity) as qty,sum( saleprice * quantity ) AS subtotal,boxsize,fkbarcodeid '," fksaleid='$saleid' AND quantity<0 GROUP BY `fkstockid`,`saleprice`");
		return $saledetails;
	}
...
//////////////////////////////////////////////////////////////////////////////////	
	function getsaleproduct_new($stockid)
	{
		global $dbname_detail;
			$sql=" SELECT itemdescription,shortdescription
			FROM 
