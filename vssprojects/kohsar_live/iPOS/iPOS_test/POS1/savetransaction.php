...
////////////////////////////////////////////////Sending Credit Sale to Accounts//////////////////////////////
$queryz = " SELECT s.pksaleid,s.fkaccountid,(SELECT (sum(sd.quantity*sd.saleprice)) as sale from  $dbname_detail.saledetail sd
where  sd.quantity > 0 and s.pksaleid=sd.fksaleid ) as sale, (SELECT (sum(sd.quantity*sd.saleprice)*-1) as returnsale from  $dbname_detail.saledetail sd
where  sd.quantity < 0 and s.pksaleid=sd.fksaleid ) as returnsale 
from $dbname_detail.sale s 
where s.pksaleid = '$saleid' and s.status = 1 and s.fkaccountid != 0    ";
		    $result_creditor		=	$AdminDAO->queryresult($queryz);
			$fkaccountid			=	$result_creditor[0]['fkaccountid'];
			$sale			=	$result_creditor[0]['sale'];
			$pksaleid			=	$result_creditor[0]['pksaleid'];
			$return			=	$result_creditor[0]['returnsale'];
			$uptame=time();
/*############################################################################################################*/			
    $creditor_link= ($crd_sale."&fkaccountid={$fkaccountid}&sale={$sale}&return={$return}&pksaleid={$pksaleid}&rid={$pksaleid}");
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    $sfields_acc	=	array("acc_link","kaltime","serverName");
    $svalues_acc	= 	array($creditor_link,$uptame,'cs');
    $AdminDAO->insertrow("$dbname_detail.accounts_links",$sfields_acc,$svalues_acc);
/*############################################################################################################*/	
$main_bal_update=($cus_baln."type=cs&fkaccountid={$fkaccountid}&sale={$sale}&return={$return}&utime={$uptame}");
/*##################################For Updating Balance##########################################################################*/	
$sfields_acc	=	array("acc_link","kaltime");
$svalues_acc	= 	array($main_bal_update,$uptame);
$AdminDAO->insertrow("$dbname_detail.accounts_links",$sfields_acc,$svalues_acc);
/*############################################################################################################*/
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//	}
