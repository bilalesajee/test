...
if($rt!=4){
 $query = "SELECT group_concat(cf.pkclosingid SEPARATOR  ',') cid,sum(cf.totalsale) sale,(SELECT sum(s.globaldiscount) as discount from $dbname_detail.sale s
...
where s.fkclosingid in ({$Closingidz}) and s.status = 1 and p.fkcctypeid!=0  and s.pksaleid=p.fksaleid  and s.countername='$rt' ) as creditcardsale,
