...
if($supp_code!=''){
$ands="and s.fksupplierid='$supp_code'";
}
/*$id = $_REQUEST['id'];
if($option!=''){
...
*/
  $query = "( SELECT 0 as return1, su.billnumber,s.fksupplierinvoiceid, FROM_UNIXTIME(su.datetime,'%d-%m-%Y') adddate, s.fksupplierid ,SUM(s.quantity * s.priceinrs) as invoice_value from $dbname_detail.supplierinvoice su
	 ...
	 where  su.datetime between '$start_date' and '$end_date'  and su.invoice_status = 1 and  su.accdatasent = 0 $ands and s.fksupplierid > 0 and  su.billnumber!='' and s.fksupplierinvoiceid > 0 and s.quantity > 0 group by s.fksupplierinvoiceid)
	 ...
	 where  su.datetime between '$start_date' and '$end_date'  and su.invoice_status = 1 and  su.accdatasent = 0 $ands and r.returnstatus = 'c' and s.fksupplierid > 0 and  su.billnumber!='' and s.fksupplierinvoiceid > 0 group by s.fksupplierinvoiceid)";
