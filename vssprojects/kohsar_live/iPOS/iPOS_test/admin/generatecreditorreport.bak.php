...
if($_SESSION['siteconfig']!=1){//edit by ahsan 16/02/2012, added if condition?>
    ...
    <?php
        ...
        { 
            $customerbalbf		=	$AdminDAO->getrows("$dbname_detail.sale s,$dbname_detail.saledetail sd,$dbname_detail.account c","sum(sd.saleprice*sd.quantity) as balbf","s.fkaccountid=id AND s.status=1 AND id='$cid' AND sd.fksaleid=pksaleid AND s.datetime < '$fromdate'");
             ...
             $query		=	"SELECT SUM(amount) amount FROM $dbname_detail.sale,$dbname_detail.payments where fkaccountid='$cid' and fksaleid=pksaleid and paymenttype='c' and paytime < '$fromdate' and status=1 group by fkaccountid";
    /*
    ...
    UNION SELECT 4 as type,pksaleid,creditinvoiceno, SUM(amount)  amount,FROM_UNIXTIME(paytime,'%d-%m-%Y') trdatetime FROM $dbname_detail.sale,$dbname_detail.chequepayment where fkcustomerid='$cid' and fksaleid=pksaleid and paymenttype='c' and paytime < '$fromdate' and status=1 group by fkcustomerid";*/
            ...
            */
            ...
            $customerinfo_		=	$AdminDAO->getrows("$dbname_detail.sale s,$dbname_detail.account c,$dbname_detail.addressbook a","s.pksaleid, s.pksaleid as pksid,s.creditinvoiceno,from_unixtime(s.datetime,'%d-%m-%Y') as trdatetime,s.globaldiscount","s.fkaccountid=id AND s.status=1 AND c.fkaddressbookid=a.pkaddressbookid AND id='$cid' AND s.datetime BETWEEN '$fromdate' AND '$todate'");
            ...
            foreach($customerinfo_ as $customerinfoval){
                ...
                $customerinfo[$i__]['creditinvoiceno']	=  $customerinfoval['creditinvoiceno'];
        ...
        $sql="SELECT s.pksaleid,s.creditinvoiceno,from_unixtime(s.datetime,'%d-%m-%Y') as trdatetime,sum(sd.saleprice*sd.quantity) as amount
                    FROM
                        $dbname_detail.sale s,$dbname_detail.saledetail sd,$dbname_detail.account c,$dbname_detail.addressbook a
                    WHERE
                        s.fkaccountid = id AND s.status=1 AND c.fkaddressbookid=a.pkaddressbookid AND id='$cid' AND sd.fksaleid=pksaleid AND s.datetime BETWEEN '$fromdate' AND '$todate' GROUP BY pksaleid
                        ";
                        
        // amount replaced by amount*rate in fcpayment by Yasir -- 11-07-11				
                
         $collectionqry="
                    SELECT paymentmethod type,pksaleid,creditinvoiceno,pkpaymentid as trid,concat('-',SUM(amount)) amount,FROM_UNIXTIME(paytime,'%d-%m-%Y') trdatetime FROM $dbname_detail.sale,$dbname_detail.payments where fkaccountid='$cid' and fksaleid=pksaleid and paymenttype='c' and paytime BETWEEN '$fromdate' AND	'$todate'  group by chequeno";
    /*
    ...
    ";
         ...
         $collectionqry="
                    SELECT paymentmethod as type,pksaleid,pkpaymentid as trid,concat('-',SUM(amount)) amount,FROM_UNIXTIME(paytime,'%d-%m-%Y') trdatetime FROM $dbname_detail.sale,$dbname_detail.payments where fkaccountid='$cid' and fksaleid=pksaleid and paymenttype='c'  and paytime BETWEEN '$fromdate' AND '$todate' 
    ";
    ...
    <strong>NTN: 01-01-2754403-6 </strong></span>
    <div style="width:8in;padding:0px;margin-left:-200px;font-size:17px;" align="center"> <img src="../images/esajeelogo.jpg" width="286" height="77"> <br />
...
global $AdminDAO;
	...
	$customersql="SELECT 
						...
						companyname
					FROM 
						$dbname_detail.customer,$dbname_detail.addressbook
						WHERE
							fkaddressbookid=pkaddressbookid AND
							pkcustomerid='$cid'";
	$custarr			=	$AdminDAO->queryresult($customersql);
	...
	if($reporttype==1)//credit account summary
	{
		$customerbalbf		=	$AdminDAO->getrows("$dbname_detail.sale s,$dbname_detail.saledetail sd,$dbname_detail.customer c","sum(sd.saleprice*sd.quantity) as balbf","s.fkaccountid=pkcustomerid AND s.status=1 AND  pkcustomerid='$cid' AND sd.fksaleid=pksaleid AND s.datetime < '$fromdate'");
		$totalsale	=	$customerbalbf[0]['balbf'];
...
UNION SELECT 4 as type,pksaleid, SUM(amount)  amount,FROM_UNIXTIME(paytime,'%d-%m-%Y') trdatetime FROM $dbname_detail.sale,$dbname_detail.chequepayment where fkaccountid='$cid' and fksaleid=pksaleid and paymenttype='c' and paytime < '$fromdate' group by fkaccountid";
		...
		$customerinfo		=	$AdminDAO->getrows("$dbname_detail.sale s,$dbname_detail.saledetail sd,$dbname_detail.customer c,$dbname_detail.addressbook a","s.pksaleid,from_unixtime(s.datetime,'%d-%m-%Y') as trdatetime,sum(sd.saleprice*sd.quantity) as amount","s.fkaccountid=pkcustomerid AND s.status=1 AND c.fkaddressbookid=pkaddressbookid AND pkcustomerid='$cid' AND sd.fksaleid=pksaleid AND s.datetime BETWEEN '$fromdate' AND '$todate' GROUP BY pksaleid");
	//collections results
