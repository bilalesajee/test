...
///////////////////////////////////////////////////////////////////////////////////
    ...
    global $AdminDAO;
        ...
        $customers		=	$AdminDAO->getrows("main.customer","ctype","pkcustomerid='$cid'");
    ...
    <?php
        ...
        { 
            ...
            $customerbalbf		=	$AdminDAO->getrows("$dbname_detail.sale s,$dbname_detail.saledetail sd,main.customer c","sum(sd.saleprice*sd.quantity) as balbf","s.fkaccountid=pkcustomerid AND s.status=1 AND pkcustomerid='$cid' AND sd.fksaleid=pksaleid AND s.datetime < '$fromdate'");
...
*/			foreach ($balance as $bal)
    ...
    $custo	=	"SELECT s.pksaleid, s.pksaleid as pksid,s.serialno,from_unixtime(s.datetime,'%d-%m-%Y') as trdatetime,s.globaldiscount  FROM $dbname_detail.sale s,main.customer c WHERE s.fkaccountid=pkcustomerid AND s.status=1 AND  pkcustomerid='$cid' AND s.datetime BETWEEN '$fromdate' AND '$todate'";
...
*/
        ...
        $sql="SELECT s.pksaleid,s.serialno,from_unixtime(s.datetime,'%d-%m-%Y') as trdatetime,sum(sd.saleprice*sd.quantity) as amount
                    ...
                    FROM
                        ...
                        $dbname_detail.sale s,$dbname_detail.saledetail sd,main.customer
...
<?php /*?><table width="399" >
  ...
  <tr>
    ...
    <td style="border:none"><strong><?php echo $retn_tot;?></strong>&nbsp;</td>
  </tr>
  <tr>
    <td style="border:none"><strong>Total Receipts</strong></td>
    <td style="border:none"><strong><?php echo $pay_tot2;?></strong>&nbsp;</td>
  </tr>
<?php */?> <?php /*?> <tr>
...
UNION SELECT 4 as type,pksaleid, SUM(amount)  amount,FROM_UNIXTIME(paytime,'%d-%m-%Y') trdatetime FROM $dbname_detail.sale,$dbname_detail.chequepayment where fkaccountid='$cid' and fksaleid=pksaleid and paymenttype='c' and paytime < '$fromdate' group by fkaccountid";
		...
		$customerinfo		=	$AdminDAO->getrows("$dbname_detail.sale s,$dbname_detail.saledetail sd,main.customer c,$dbname_detail.addressbook a","s.pksaleid,from_unixtime(s.datetime,'%d-%m-%Y') as trdatetime,sum(sd.saleprice*sd.quantity) as amount","s.fkaccountid=pkcustomerid AND s.status=1 AND c.fkaddressbookid=pkaddressbookid AND pkcustomerid='$cid' AND sd.fksaleid=pksaleid AND s.datetime BETWEEN '$fromdate' AND '$todate' GROUP BY pksaleid");
