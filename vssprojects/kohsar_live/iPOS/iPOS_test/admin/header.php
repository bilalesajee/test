...
<div id="notice" style="display:none"></div>
   ...
   <div id="header_right"></div>
		...
		for($i=0;$i<sizeof($sectionres);$i++)
		{
			...
			// temp check
			if(!in_array($sectionid, array(2, 15)) and $_SESSION['addressbookid'] == 1928)
			{
				continue;
			}
        ...
        <div style="float:right;margin:5px 10px 0 0;">
           <!-- <a href="#" onclick="getUpdat()"  ><span style="position:absolute;margin:1px 0 0 -92px;"><img src="images/updates.png" border="0" /></span></a>--> <a href="../signout.php">
                <img src="../images/signout.png" alt=""/>
            </a>
        </div>
        <div style="float:right;margin:5px 10px 0 0; font-size:12px; font-weight:bold; background-color:#BBD9EE">
           <!-- <a href="#" onclick="getUpdat()"  ><span style="position:absolute;margin:1px 0 0 -92px;"><img src="images/updates.png" border="0" /></span></a>--> 
         <?php 
		$query = "select * from main_kohsar.messages where status =0 and  (to_user = '{$_SESSION['addressbookid']}' or from_user='{$_SESSION['addressbookid']}')";
$reportresult = $AdminDAO->queryresult($query);
$count = count($reportresult);
		 ?> 
           <input type="button" class="datepicker_control" onclick="openWin()" value="Inbox(<?php echo $count;?>)" />            
            </a>
        </div>
