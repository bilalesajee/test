...
$query = "SELECT p.pkadjustmentid, p.fksupplierid,FROM_UNIXTIME(p.addtime,'%d-%m-%Y') adddate ,SUM(d.value) as value ,sl.pksupplierid, sl.companyname from $dbname_detail.adjustment p
	 left join $dbname_detail.adjustment_detail d on d.fkadjustmentid = p.pkadjustmentid 
	 left join $dbname_main.supplier sl on p.fksupplierid=sl.pksupplierid
	 where  $cond  group by sl.pksupplierid,d.fkadjustmentid    ";
