...
$query	= "SELECT
		...
		SUM( s.unitsremaining ) AS unitsremaining,

		(IF (min(s.expiry) = '0', '--------',min(FROM_UNIXTIME(s.expiry,'%d-%m-%y')))) AS minexpiry	,

		round(MAX(s.retailprice),2) as currprice, 

		round(MAX(s.priceinrs),2)  as tradeprice,
	...
	FROM 
		...
		$dbname_detail.stock s left join $dbname_detail.stock_adjustment_detail sad on (s.fkbarcodeid=sad.fkbarcodeid),
