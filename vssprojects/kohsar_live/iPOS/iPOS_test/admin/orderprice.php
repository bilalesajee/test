...
$query	=	"SELECT
				...
				exchangerate
			FROM 
				`order` sl,orderpurchase p, shipment LEFT JOIN currency ON (shipmentcurrency=pkcurrencyid)
			WHERE
...
<div id="shipfrmdiv" style="display: block;"> <br>
  <form id="shipmentfrm" style="width: 920px;" action="orderpriceact.php?id=-1" class="form"  enctype="multipart/form-data">
    <fieldset>
      <legend>
      Shipment
      Pricing</legend>
        ...
        <tr>
          <td height="12" valign="top"><div class="topimage2" style="height:6px;"><!-- --></div>
            <table style="width:895px;" cellpadding="2" cellspacing="0">
              ...
              {
				  $status		=	0;
				  $orderid		=	$receivedata[$i]['pkorderid'];
				  $purchaseid	=	$receivedata[$i]['pkorderpurchaseid'];
				  // building statuses for further processing
				  $statusres	=	$AdminDAO->getrows("orderprice,addressbook","concat(firstname,' ',lastname) pricedby,costprice,retailpercentage,retailprice,shipmentcharges,shipmentpercentage","fkaddressbookid=pkaddressbookid AND fkshipmentid='$id' AND fkorderpurchaseid='$purchaseid'");
				 /* echo "<pre>";
				  print_r($statusres);
				  echo "</pre>";*/
              ...
              <tr bgcolor="<?php echo $bgcolor;?>"><div id="rate" style="display:none;"><?php echo $receivedata[0]['exchangerate'];?></div>
                ...
                <td><?php if($status==1){ echo $statusres[0]['shipmentpercentage'];?><input type="hidden" size="5" name="shipmentpercentage[]" id="shipmentpercentage_<?php echo $i;?>" value="" /><?php } else{;?><input type="text" size="5" name="shipmentpercentage[]" id="shipmentpercentage_<?php echo $i;?>" onkeypress="return isNumberKey(event)" onchange="cal(<?php echo $i;?>,<?php echo $receivedata[$i]['purchaseprice'];?>)"  /><?php }?></td>
                <td><?php if($status==1){ echo $statusres[0]['shipmentcharges'];?><input type="hidden" size="5" name="shipmentcharges[]" id="shipmentcharges_<?php echo $i;?>" value=""/><?php } else{;?><input type="text" size="5" name="shipmentcharges[]" id="shipmentcharges_<?php echo $i;?>" onkeypress="return isNumberKey(event)" /><?php }?></td>
                <td><?php if($status==1){ echo $statusres[0]['costprice'];?><input type="hidden" size="5" name="costprice[]"  id="costprice_<?php echo $i;?>" value=""/><?php } else{;?><input type="text" size="5" name="costprice[]"  id="costprice_<?php echo $i;?>"  onkeypress="return isNumberKey(event)" /><?php }?></td>
                <td><?php if($status==1){ echo $statusres[0]['retailpercentage'];?><input type="hidden" size="5" name="retailpercentage[]" id="retailpercentage_<?php echo $i;?>" /><?php } else{;?><input type="text" size="5" name="retailpercentage[]" id="retailpercentage_<?php echo $i;?>"   onkeypress="return isNumberKey(event)"  onchange="rcalc(<?php echo $i;?>,this.value,<?php echo $stsize;?>)" /><?php }?></td>
                <?php
				...
				?>
                <td><?php if($status==1){ echo $storeprice;?><input type="hidden" size="5" name="store<?php echo $storesinfo[$k][pkstoreid];?>[]" id="store_<?php echo $i."_".$k;?>" value=""/><?php } else {?><input type="text" size="5" name="store<?php echo $storesinfo[$k][pkstoreid];?>[]" id="store_<?php echo $i."_".$k;?>" onkeypress="return isNumberKey(event)" /><?php } ?></td>
				<?php 
                ...
                <td><?php if ($status==1){ echo "Prepared by ".$statusres[0]['pricedby'];} else { echo "Not Priced";}?><input type="hidden" name="purchaseid[]" value="<?php echo $receivedata[$i]['pkorderpurchaseid'];?>" /><input type="hidden" name="orderid[]" id="orderid" value="<?php echo $orderid;?>" /></td>
...
function priceresponse(text)
{
	if(text=='')
	{
		adminnotice('Items have been Priced Successfully.',0,5000);
		hidediv('shipfrmdiv');
		jQuery('#maindiv').load('manageshipmentclosed.php');
	}
	else if(text=='order moved')
	{
		adminnotice('Items have been Priced and Unpurchased orders of this Shipment has been moved to a new Shipment.',0,5000);
		hidediv('shipfrmdiv');
		jQuery('#maindiv').load('manageshipmentclosed.php');		
	}
	else
...
function viewhistory(bc)
{
	var display='toolbar=0,location=0,directories=1,menubar=1,scrollbars=1,resizable=1';
 		window.open('orderpricehistory.php?bc='+bc,'Price History',display); 
}
</script>