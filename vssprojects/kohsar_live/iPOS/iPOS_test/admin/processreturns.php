...
function submitfrm(trg)
{
	...
	options	=	{	
					url : 'processreturnsact.php?action='+trg,
					type: 'POST',
					success: response2
				}
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" >
  <tr>
    <td style="font-weight:bold; color:#0000FF; font-size:14px;">Press Mark As Close Button For Stock Return otherwise quantity will not be subtracted </td>
  </tr>
</table>
...
<table>
    ...
    <tr>
        ...
        <td><select name="returnstatus[]" id="returnstatus"  style="width:150px;"><option value="p">Pending</option><option value="c">Confirmed</option></select></td>
        <td><input type="text" class="text" name="return[]" id="return<?php echo $i+1;?>" value="<?php echo $retqty;?>" maxlength="10" /><input type="hidden" name="stockid[]" id="stockid<?php echo $i+1;?>" value="<?php echo $pkstockid;?>" /><input type="hidden" name="totalrecs" id="totalrecs" value="<?php echo $totalrecs;?>" /><input type="hidden" name="urem[]" id="urem" value="<?php echo $remaining;?>" /><input type="hidden" name="fkinvid[]" id="fkinvid" value="<?php echo $fksuplierinvid;?>" /><input type="hidden" name="fkbarid[]" id="fkbarid" value="<?php echo $fkbarid;?>" /><input type="hidden" name="srcid[]" id="srcid" value="<?php echo $srcid;?>" /></td>
    </tr>
    <?php
	}//for
	?>
