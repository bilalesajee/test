...
if($_SESSION['siteconfig']!=1 && strstr($_SERVER['HTTP_REFERER'],'sectionid=3')==true){//edit by ahsan 17/02/2012, added if condition //run this block for local(store) or global(main&store)
	...
	$sql	=	"
								...
								FROM 
									...
									$dbname_detail.stock LEFT JOIN addressbook ON (fkemployeeid=pkaddressbookid),shipment LEFT JOIN currency ON shipmentcurrency=pkcurrencyid,barcode
	...
	<?php
...

		{
			...
			<tr>
                ...
                <td align="right"><?php echo number_format($result[$i]['costprice'],2);?></td>

				<td align="right"><?php echo number_format($result[$i]['priceinrs'],2);?></td>

				<td align="right"><?php echo number_format($unitsprice,2);?></td>
                 <td align="right"><?php echo number_format($unitscostprice,2);?></td>
				<td align="right"><?php echo number_format($result[$i]['retailprice'],2);?></td>
	...
	<tr>
			  ...
			  <td align="right"><b><?php echo number_format($totalpriceinrs,2);?></b></td>
...
?>
    ...
    <?php $res=get_shipment_report_data(); ?>
