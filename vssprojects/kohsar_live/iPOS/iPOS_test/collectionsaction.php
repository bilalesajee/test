...
//////////////////////////////////////////////////////////////////////////////////////////////
/////////////////////////Check for Extra Payment By Fahad 27-12-2012//////////////////////////
$paymentmethod	=	$_POST['paymentmethod'];
$cash_given	 	=	$_POST['amount1'];

$remaining_balance		=	$_POST['remainingprice'];
/*if($cash_given > $remaining_balance){
echo "Your Amount Entered is Greater than Remainaing Balance";	
exit;
	}*/
////////////////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////////////////	
...

	{
		...
		{ //echo $cash.' - ';
...

			{
				...
				$Bill->updatepayment($fcpaid*$fcrate,$paymentmethod,$saleid); // $paid*$fcrate replaced from $paid by Yasir -- 06-07-11	//$paid replaced by $fcpaid by ahsan 07/03/2012			

				$cash	=	$cash-($fcpaid*$fcrate); // $paid replaced by $paid*$fcrate by Yasir -- 19-07-11 //$paid replaced by $fcpaid by ahsan 07/03/2012

				

				// if paid is greater than remaining -- added by Yasir -- 18-07-11

				if ( ($fcpaid*$fcrate) > $remainingprice ){//$paid replaced by $fcpaid by ahsan 07/03/2012

					$adjust_amount	=	$remainingprice	-	($fcpaid*$fcrate);//$paid replaced by $fcpaid by ahsan 07/03/2012
...
if($collectionid!=''){
 $tam=time();
  $cust_new=($serverUrl_send.$customerid.'&amount='.$cashpaid.'&paymentmethod='.$paymentmethod.'&charges='.$creditcharges.'&bank='.$bank.'&chequedate='.$chequedate.'&chequeno='.$chequenumber.'&ccno='.$ccno);
 /*############################################################################################################*/	
$sfields_acc	=	array("acc_link","kaltime");
$svalues_acc	= 	array($cust_new,time());
$AdminDAO->insertrow("$dbname_detail.accounts_links",$sfields_acc,$svalues_acc);
 /*############################################################################################################*/

            
			if($_SESSION['SERVER_ACC_ONLINE']!=1){
			$fields_cust    =       array('fkaccountid','fksaleid','amount','paymentmethod','charges','fkbankid','chequedate','chequeno','ccno','datetime','datasent'); 
            $data_cust      =       array($customerid,$saleid,$cashpaid,$paymentmethod,$creditcharges,$bank,$chequedate,$chequenumber,$ccno,$tam,0);
              
			   }else{
			$fields_cust    =       array('fkaccountid','fksaleid','amount','paymentmethod','charges','fkbankid','chequedate','chequeno','ccno','datetime','datasent'); 
            $data_cust      =       array($customerid,$saleid,$cashpaid,$paymentmethod,$creditcharges,$bank,$chequedate,$chequenumber,$ccno,$tam,1);
           	   
				   }
		    
		   
		   
		    $AdminDAO->insertrow("$dbname_detail.collection4acc",$fields_cust,$data_cust);
