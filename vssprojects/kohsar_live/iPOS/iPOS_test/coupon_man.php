...
if($delid!='' && $oper=='del')
{
		...
		foreach($ids as $value)
		{
			if($value!='')
			{
				$delcondition =" pkcouponid  = '$value' ";
				$AdminDAO->deleterows("$dbname_detail.coupon_management",$delcondition,1);
			}
		}
}
//************ DUMMY SET ***************
$labels = array("ID", "Coupon ID","Amount ","paymentmethod","Reason","Status");
...
$navbtn	=	"";
	 ...
	 $navbtn .="<a class=\"button2\" id=deletebrands onmouseover=\"buttonmouseover(this.id)\" onmouseout=\"buttonmouseout(this.id)\" href=javascript:deleterecords('$dest','$div','$_SESSION[qs]') title=\"Delete Reasons\"><span class=\"editrecord\"></span></a>";*/
