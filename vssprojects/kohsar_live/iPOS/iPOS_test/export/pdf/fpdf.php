...
class PDF extends FPDF
{
	...
	function FancyTable($header,$data)
	{
		...
		{
			$wHeader = $this->GetStringWidth($header[$k]);
			$w[$k] = $wHeader;
			$w[$k] = round($w[$k]);
		}
		...
		if ($tableWidth<$totalWidth)
		{
			if ($headerLen>0)
			{
				$fark = ($totalWidth - $tableWidth) / $headerLen;
				for($k=0;$k<$headerLen;$k++) $w[$k] += $fark;
			}
		}

		//Colors, line width and bold font
		$this->SetFillColor(75,95,132);
		$this->SetTextColor(255);
		$this->SetDrawColor(170,187,221);
		$this->SetLineWidth(.3);
		$this->SetFont('','B',9);
		//Header
		...
		for($i=0;$i<$rowLen;$i++)
		{
			$countLen = count($data[$i]);
			for($k=0;$k<$countLen;$k++)
			{
				$this->Cell($w[$k],6,$data[$i][$k],'LR',0,'L',$fill);
			}
			$this->Ln();
			$fill=!$fill;
		}		
