...
session_start();
?>
<div id="quotetitlediv"><div style="position:absolute;background-color:#000;padding:0px;-moz-border-radius:3px;-webkit-border-radius:4px;font-weight:bold;color:#fff;float:left;" ><?php if($_SESSION['quotetitle'] != '') { echo $_SESSION['quotetitle']; } ?></div></div>
<div id="user">
<?php
...
$name			=	$_SESSION['name'];
$userid			=	$_SESSION['addressbookid'];
$closingsalesarray	=	$AdminDAO->getrows("$dbname_detail.closingsales","fksaleid"," countername='$countername' ");
...
$sale			=	$AdminDAO->getrows("$dbname_detail.sale","count(*) as bills", "fkuserid = '$userid' AND fkclosingid='$closingsession' AND status='1' $andclosed  AND countername='$countername' ");
...
?>
   ...
   <?php
	$returnquery	=	"SELECT MAX(pksaleid) as saleid FROM $dbname_detail.sale WHERE fkuserid = '$userid' AND fkclosingid='$closingsession' AND status='1' AND countername='$countername'";
	$returnqueryres	=	$AdminDAO->queryresult($returnquery);
	$saleid			=	$returnqueryres[0]['saleid'];
   	$adjustquery	=	"SELECT ((IF(sum(tendered)IS NULL,0,sum(tendered)))-(IF(sum(amount)IS NULL,0,sum(amount)))) as adjustment FROM $dbname_detail.payments ,$dbname_detail.sale s1 WHERE s1.pksaleid =  '$saleid' AND pksaleid = fksaleid
						";
...
<div id="creditcutomerdiv"><div style="position:absolute;background-color:#000;padding:0px;-moz-border-radius:3px;-webkit-border-radius:4px;font-weight:bold;color:#fff;float:right;margin-left:600px;" ><?php if($creditcustomername!=''){ echo "Credit for: ".$creditcustomername;}?></div></div>