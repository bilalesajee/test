...
function suggestnow_mobile(e,acdivid,v1)
{
...
function autosuggest_mobile(v) 
{
	...
	{
		q = document.getElementById('itemname').value;
	}	
	//alert(document.getElementById('suggestdiv').innerHTML);
//	alert(q+' is the q'+v);
	// Set te random number to add to URL request
	...
	{
		nocache = Math.random();
		http.open('get', 'includes/autocomplete/search_mob.php?q='+q+'&nocache = '+nocache);
		http.onreadystatechange = autosuggestReply;
		http.send(null);
	}
	else
...
function getlitext_mobile(current,barcode,itemdesp)
{
	var litext	=	document.getElementById(current).textContent;
	//var val	=	document.getElementById(barcode).value;
	
	//alert(barcode);
	document.getElementById('itemname').value=itemdesp;
	document.getElementById('barcde').value=barcode;
	//document.getElementById('quantity').focus();
