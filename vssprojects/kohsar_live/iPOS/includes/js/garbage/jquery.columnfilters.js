		...
		return this.each(function() {
			...
			$('tbody:first tr:first td', obj).each(
				function(iColCount) {
					...
					$(filterBox).keyup(runFilters);
					$(filterColumn).append(filterBox);
					$(filterRow).append(filterColumn);
					
					if (settings.excludeColumns.length) {
			...
			function runFilters(event) {			
				$('input._filterText', obj).each(
					function(iColCount) {
						...
						if (settings.notCharacter && sFilterTxt.indexOf(regexEscape(settings.notCharacter)) === 0) {
							sFilterTxt = sFilterTxt.substr(regexEscape(settings.notCharacter).length,sFilterTxt.length);
			...
			function genAlternateClassNames() {
				if (settings.alternateRowClassNames) {
					...
					$('tbody:first tr', obj).each(
						function() {
							if ($(this).css('display') !== 'none') {
								...
								$(this).addClass((bPos) ? sFirst : sSecound);
								bPos = !bPos;
							}
						}
					);
			...
			function regexEscape(text, omit) {
				var specials = [
					'/', '.', '*', '+', '?', '|',
					'(', ')', '[', ']', '{', '}', '\\'
					];
				...
				return text.replace(escapePatt, '\\$1');
			}
		});
	};
